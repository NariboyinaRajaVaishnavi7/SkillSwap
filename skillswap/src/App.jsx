...
const API_URL = "https://skillswap1-akcc.onrender.com";
...
function Browse({ setPage, setSelectedUser, addToast, user }) {
  ...
  const fetchConnections = () => {
    const token = localStorage.getItem("token");
    fetch("https://skillswap1-akcc.onrender.com/connections", {
      headers: { "Authorization": `Bearer ${token}` }
    })
      .then(res => res.json())
...
function Messages({ setPage, selectedUser, user }) {
  ...
  useEffect(() => {
    socketRef.current = io("https://skillswap1-akcc.onrender.com");
...
function Schedule({ setPage, addToast }) {
  ...
  useEffect(() => {
    ...
    fetch("https://skillswap1-akcc.onrender.com/connections", { headers: { "Authorization": `Bearer ${token}` } })
      .then(res => res.json())
      .then(data => {
        const accepted = data.filter(c => c.status === "accepted");
